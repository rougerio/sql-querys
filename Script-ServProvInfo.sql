...
SELECT * FROM (
SELECT SERVPROV, RAZON_SOCIAL, NVL(SCAC, ' ') SCAC, NVL(SC, ' ') SC, TARIFA_ACTIVA, INSERT_DATE
FROM (
    SELECT SERVPROV_XID SERVPROV, RAZON_SOCIAL, 
    SCAC, SC, IS_ACTIVE TARIFA_ACTIVA, 
    ROW_NUMBER() OVER(PARTITION BY SERVPROV_XID,
    RAZON_SOCIAL, SCAC, SC ORDER BY SERVPROV_XID, IS_ACTIVE DESC) AS CON_TARIFA,
    INSERT_DATE
    FROM (
...
WHERE 1 = 1 
--AND TARIFA_ACTIVA = 'Y'
--AND INSERT_DATE >= TO_DATE('01/09/2025', 'DD/MM/YYYY')
;
/*
