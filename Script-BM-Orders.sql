...
AND NOT EXISTS (SELECT ORR.ORDER_RELEASE_GID FROM ORDER_RELEASE_REMARK ORR WHERE ORR.ORDER_RELEASE_GID = ORE.ORDER_RELEASE_GID AND ORR.REMARK_QUAL_GID = 'SHIP_DATE')  
AND ORS.STATUS_VALUE_GID IN ('NBL/MX.PLANNING_PLANNED - FINAL','NBL/MX.PLANNING_EXECUTED - FINAL')  
AND ORE.RATE_GEO_GID LIKE '%BKHL%'  
