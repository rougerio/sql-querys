...
SELECT T.RATE_GEO_GID,
T.RATE_GEO_XID,
T.RATE_OFFERING_GID, 
T.SOURCE_REGION_GID, 
T.DEST_REGION_GID, 
T.X_LANE_GID OLD_X_LANE_GID,
XL1.X_LANE_GID NEW_X_LANE_GID
FROM (
        ...
        FROM RATE_GEO RG INNER JOIN 
        X_LANE XL ON RG.X_LANE_GID = XL.X_LANE_GID
        WHERE NOT REGEXP_LIKE(XL.X_LANE_XID, '^[[:digit:]]+$')
        AND RG.DOMAIN_NAME = 'NBL/MX'
        AND RG.IS_ACTIVE = 'Y'
) XL1 ON T.SOURCE_REGION_GID  = XL1.SOURCE_REGION_GID AND T.DEST_REGION_GID = XL1.DEST_REGION_GID
ORDER BY T.RATE_GEO_GID, XL1.X_LANE_GID




NBL/MX.ORIENTE|MXC|GDL|25

SELECT Geo
FROM (
        SELECT SUBSTR(RG.RATE_GEO_XID, INSTR(RG.RATE_GEO_XID, '|') + 1, LENGTH(RG.RATE_GEO_XID) - INSTR(RG.RATE_GEO_XID, '|') + 1 ) Geo 
        FROM RATE_GEO RG INNER JOIN 
        X_LANE XL ON RG.X_LANE_GID = XL.X_LANE_GID
        WHERE REGEXP_LIKE(XL.X_LANE_XID, '^[[:digit:]]+$')
        AND RG.DOMAIN_NAME = 'NBL/MX'
        AND RG.IS_ACTIVE = 'Y'
        AND XL.DEST_REGION_GID IS NOT NULL
)
GROUP BY Geo
ORDER BY Geo
...
SELECT CustomerID, 
       SUBSTR(CustomerNotes, INSTR(CustomerNotes, 'PO:') + 4) AS RestOfString
FROM dbo.Customers
WHERE INSTR(CustomerNotes, 'PO:') > 0;