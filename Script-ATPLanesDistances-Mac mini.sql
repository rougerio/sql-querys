...
SELECT * FROM (
    SELECT * FROM (
        SELECT OTM.LOCATION_GID, OTM.LOCATION_XID, OTM.LAT, OTM.LON, 
        --RC.DESTINATION_LOCATION_XID, 
        TRUNC(RC.DESTINATION_LATITUDE, 5) RC_LAT, TRUNC(RC.DESTINATION_LONGITUD, 5) RC_LON--, 
...
GROUP BY LOCATION_GID, LOCATION_XID, LAT, LON, RC_LAT, RC_LON
ORDER BY LOCATION_GID;

SELECT * FROM LOCATION_OTM_25
WHERE DOMAIN_NAME = 'NBL';

SELECT DESTINATION_LONGITUD,TRUNC(DESTINATION_LONGITUD,5), TO_CHAR(DESTINATION_LONGITUD, ‘fm99D0000’) 
FROM DISTANCE_BY_LOCATION_LOCATION
WHERE DESTINATION_LOCATION_XID = 'CUS-1034553-103021';
