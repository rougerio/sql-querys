...
SHIP.REMARK_TEXT, 
ORDE.REMARK_TEXT AS ORDER_REMARK_TEXT, 
SHIP.DOMAIN_NAME 
FROM (
    SELECT ROW_NUMBER() OVER(PARTITION BY SK.SHIPMENT_GID ORDER BY SK.REMARK_SEQUENCE ASC) AS SK_ROW_ID, 
        ...
        SK.REMARK_TEXT, 
        SK.DOMAIN_NAME, 
        SUBSTR(SK.REMARK_TEXT, 1, INSTR(REMARK_TEXT, '--') - 1) AS ITEM_DESC, 
        SUBSTR(SK.REMARK_TEXT, INSTR(REMARK_TEXT, '--') + 2) AS ITEM_COUNT 
    FROM SHIPMENT_REMARK SK  
    WHERE REMARK_QUAL_GID LIKE 'NBL.ITEM_DESC_AND_COUNT%' 
) SHIP INNER JOIN ( 
...
ORDER BY ORE_ROW_ID 
) ORDE ON ORDE.SHIPMENT_GID = SHIP.SHIPMENT_GID AND SHIP.SK_ROW_ID = ORDE.ORE_ROW_ID AND SHIP.ITEM_DESC = ORDE.ITEM_DESC 
WHERE 1 = 1 
AND SHIP.REMARK_TEXT <> ORDE.REMARK_TEXT

AND SHIP.SHIPMENT_GID = 'NBL.NB53976928'
