...
----Invoce
SELECT I_TRANSMISSION_NO, I_TRANSACTION_NO, STATUS, ELEMENT_NAME, OBJECT_GID, InvoiceNum
FROM (
        SELECT ITRAC.I_TRANSACTION_NO,
        ITRAC.I_TRANSMISSION_NO,
        ITRAC.STATUS,
        ITRAC.ELEMENT_NAME,
        ITRAC.OBJECT_GID, 
        ...
        AND TO_CHAR(ITMS.INSERT_DATE, 'MM/DD/YYYY') BETWEEN '04/01/2026' AND '04/16/2026'
        AND ITMS.DOMAIN_NAME = 'NBL'
    ) T
WHERE InvoiceNum IN ('275849');
...
FROM I_TRANSMISSION ITMS INNER JOIN 
    I_TRANSACTION ITRAC ON ITMS.I_TRANSMISSION_NO = ITRAC.I_TRANSMISSION_NO 
WHERE ELEMENT_NAME = 'Invoice' 
AND TO_CHAR(ITMS.INSERT_DATE, 'MM/DD/YYYY') BETWEEN '01/01/2026' AND '04/16/2026' 
AND ITMS.DOMAIN_NAME = 'NBL' 
